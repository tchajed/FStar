...
| FStar_Parser_AST.TopLevelLet (_84_79, _84_81, pats) -> begin
(
...
let termty' = (FStar_List.map (fun _84_91 -> (match (_84_91) with
| (p, t) -> begin
(Prims.strcat p (Prims.strcat ":" t))
end)) termty)
in (Prims.strcat "let " (FStar_String.concat ", " termty'))))
end
| FStar_Parser_AST.Main (_84_94) -> begin
"main ..."
end
| FStar_Parser_AST.Assume (_84_97, i, t) -> begin
...
| FStar_Parser_AST.Tycon (_84_103, tys) -> begin
(let _181_80 = (let _181_79 = (FStar_All.pipe_right tys (FStar_List.map (fun _84_109 -> (match (_84_109) with
| (t, d) -> begin
...
| FStar_Parser_AST.Val (_84_111, i, t) -> begin
...
| FStar_Parser_AST.Exception (i, _84_118) -> begin
...
| FStar_Parser_AST.SubEffect (_84_172) -> begin
"sub_effect"
end
| FStar_Parser_AST.Pragma (_84_175) -> begin
"pragma"
end
| FStar_Parser_AST.Fsdoc (comm, _84_179) -> begin
comm
end))
...
| FStar_Parser_AST.TyconRecord (_84_194, _84_196, _84_198, fields) -> begin
(FStar_List.existsb (fun _84_205 -> (match (_84_205) with
| (_id, _t, doco) -> begin
(FStar_Util.is_some doco)
end)) fields)
end
| FStar_Parser_AST.TyconVariant (_84_207, _84_209, _84_211, vars) -> begin
(FStar_List.existsb (fun _84_219 -> (match (_84_219) with
| (_id, _t, doco, _u) -> begin
(FStar_Util.is_some doco)
end)) vars)
end))
in (FStar_List.existsb (fun _84_222 -> (match (_84_222) with
| (tycon, doco) -> begin
...
| FStar_Parser_AST.Fsdoc (_84_229) -> begin
true
end
| FStar_Parser_AST.Tycon (_84_232, ty) -> begin
(tycon_documented ty)
end
| _84_237 -> begin
false
...
let _84_244 = d
in (match (_84_244) with
| {FStar_Parser_AST.d = decl; FStar_Parser_AST.drange = _84_242; FStar_Parser_AST.doc = fsdoc} -> begin
(

let _84_245 = (let _181_103 = (let _181_102 = (string_of_decl' d.FStar_Parser_AST.d)
in (code_wrap _181_102))
in (w _181_103))
in (

let _84_253 = (match (fsdoc) with
| Some (doc, _kw) -> begin
(w (Prims.strcat "\n" doc))
end
| _84_252 -> begin
...
| FStar_Parser_AST.TopLevelModule (_84_258) -> begin
(match (topdecl.FStar_Parser_AST.doc) with
| Some (doc, kw) -> begin
(match ((FStar_List.tryFind (fun _84_266 -> (match (_84_266) with
| (k, v) -> begin
(k = "summary")
end)) kw)) with
| None -> begin
...
let _84_290 = (match (m) with
| FStar_Parser_AST.Module (n, d) -> begin
((n), (d), ("module"))
end
| FStar_Parser_AST.Interface (n, d, _84_284) -> begin
((n), (d), ("interface"))
end)
in (match (_84_290) with
...
let _84_302 = (document_toplevel name top_decl)
in (match (_84_302) with
...
let _84_311 = (let _181_110 = (FStar_Util.format "# module %s" ((name.FStar_Ident.str)::[]))
in (w _181_110))
in (

let _84_313 = (let _181_111 = (FStar_Util.format "%s\n" ((summary)::[]))
in (w _181_111))
in (

let _84_315 = (let _181_112 = (FStar_Util.format "%s\n" ((comment)::[]))
in (w _181_112))
in (

let _84_317 = (FStar_List.iter (document_decl w) other_decls)
in (

let _84_319 = (FStar_Util.close_file fd)
in name)))))))
end)))))))
end
| None -> begin
...
let _84_329 = (FStar_List.iter (fun m -> (let _181_119 = (FStar_Util.format "%s\n" ((m.FStar_Ident.str)::[]))
